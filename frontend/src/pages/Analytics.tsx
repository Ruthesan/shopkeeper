...
export default function Analytics() {
  ...
  const totalProfit  = chart.reduce((s, d) => s + d.profit,  0)   // Feature 2
  const avgDaily     = totalRevenue / (chart.length || 1)
  const bestDay      = chart.reduce(
    (best, d) => d.revenue > best.revenue ? d : best,
    chart[0] ?? { revenue: 0, day: '', units: 0, profit: 0 },
  )

  // Feature 2: overall margin % for the selected period
  ...
  return (
    <div className="gap-24">
      ...
      {/* Stat row — Feature 2: total profit + margin */}
      <div className="stat-grid">
        ...
        <div className="stat-card">
          <div className="stat-label">Total Profit ({days}d)</div>
          <div className="stat-value" style={{ color: totalProfit >= 0 ? 'var(--naira)' : 'var(--danger)' }}>
            {fmt(totalProfit)}
          </div>
          {marginPct && (
      ...
      {/* Revenue + Profit area chart — Feature 2 */}
      <div className="card">
        <div className="section-title">Revenue vs Profit — Last {days} Days</div>
        {chart.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={chart} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Area
                  ...
                  stroke="#34d97b" fill="url(#profGrad2)"
      ...
      <div className="grid-2">
          ...
          {chart.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={210}>
              <BarChart data={chart} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                <Tooltip {...CHART_STYLE} formatter={(v: number) => [v, 'Units']} />
        ...
        {/* Top sellers — Feature 2: profit column added */}
        <div className="card">
                  ...
                  <div key={s.product_id}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                        <span style={{ fontSize: 11, fontWeight: 800, fontFamily: 'var(--font-mono)', color: rankColor, width: 18, flexShrink: 0 }}>
                          #{i + 1}
                        </span>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 8 }}>
                        <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: 'var(--naira)', fontWeight: 600 }}>
                          {fmt(s.total_revenue)}
                        </div>
                        {/* Feature 2: profit per top seller */}
                        <div style={{ fontSize: 11, color: 'var(--success)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>
                          {fmt(s.total_profit)} profit
                        </div>
                      </div>
                    </div>
